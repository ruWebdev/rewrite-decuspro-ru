...
<template>
    ...
    <Head :title="`Категории: ${props.site.name}`" />
    ...
    <div class="min-h-screen bg-white text-gray-900">
        ...
        <main class="mx-auto max-w-5xl px-4 py-6 space-y-4">
            <h1 class="text-2xl font-semibold">Категории сайта: {{ props.site.name }}</h1>

            <div class="flex items-center justify-between">
                <p class="text-sm text-gray-600">
                    Загрузка категорий с Joomla по URL: {{ props.site.url }}
                </p>

                <Link :href="route('sites.categories.sync', props.site.id)" method="post" as="button"
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        <tr v-if="!props.categories.length">
                            <td colspan="3" class="px-4 py-6 text-center text-sm text-gray-500">
                                Категорий пока нет. Нажмите «Получить / Обновить», чтобы загрузить их из Joomla.
                            </td>
                        </tr>
                    </tbody>
                </table>
